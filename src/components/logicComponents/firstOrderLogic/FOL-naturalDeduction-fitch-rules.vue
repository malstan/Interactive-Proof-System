<template>
  ...
  <q-card class="my-card q-my-lg" v-if="userInput">
    ...
    <q-card-section v-if="rulesWithTermNeed.includes(currentRule.value)">
      <FormulaInput
        v-model="choosenTerm"
...
import formulaGrammar from "src/logic/firstOrderLogic/firstOrderGrammar";
import termGrammar from "src/logic/firstOrderLogic/firstOrderTermGrammar";
import { applyNdFitchRule } from "src/logic/firstOrderLogic/firstOrderFormulaProcessor";

import rules from "src/logic/firstOrderLogic/rules/naturalDeductionFitchRules";
...
function isUserInputValid(userInputs) {
  ...
  if (
    ...
    return false;
  // check if rule that needs term has term
  if (
    rulesWithTermNeed.includes(currentRule.value.value) &&
    userInputs.term == ""
  )
    return false;

  // check if assupmtion has formula exists
...
async function getUserInput() {
  ...
  const result = await new Promise((resolve, reject) => {
    const unwatch = watch(inputSubmited, (input) => {
      ...
      if (input) {
        ...
        resolve({
          formula: choosenFormula.value,
          term: choosenTerm.value,
          rows: choosenRows.value,
        });
        unwatch();
      }
      // canceled
  ...
  if (result && !result.formula.includes(",")) {
    // check syntax
    const formulaParser = new nearley.Parser(
      nearley.Grammar.fromCompiled(formulaGrammar)
    );
    const termParser = new nearley.Parser(
      nearley.Grammar.fromCompiled(termGrammar)
    );
    try {
      formulaParser.feed(result.formula.replaceAll(" ", ""));
    } catch (error) {
      Notify.create({
        type: "negative",
    ...
    try {
      termParser.feed(result.term.replaceAll(" ", ""));
    } catch (error) {
      Notify.create({
        type: "negative",
    ...
    return { formula: result.formula, rows: result.rows, term: result.term };
  } else {
    if (result != false)
      Notify.create({
        type: "negative",
