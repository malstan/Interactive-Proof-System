 ...
 *    GSK - gentzen sequent calculus
 * notations:
 *    tree
 *    fitch
 */

export const PLStore = defineStore("propositionalLogicStore", {
  state: () => ({
    currentMethod: "GSK",
  ...
  actions: {
    setFromCookies() {
      Cookies.has("pl-formula") &&
        this.setInputFormula(Cookies.get("pl-formula"));

      Cookies.has("pl-leaves") && this.setLeaves(Cookies.get("pl-leaves"));

      Cookies.has("pl-proof") && (this.proof = Cookies.get("pl-proof"));

      Cookies.has("pl-history") && this.setHistory(Cookies.get("pl-history"));

      Cookies.has("pl-hypo") && (this.hypotesis = Cookies.get("pl-hypo"));

      Cookies.has("pl-method")
        ? (this.currentMethod = Cookies.get("pl-method"))
        : Cookies.set("pl-method", this.currentMethod);

      Cookies.has("pl-notation")
        ? (this.currentNotation = Cookies.get("pl-notation"))
        : Cookies.set("pl-notation", this.currentNotation);

      if (Cookies.has("pl-fitch")) {
        this.conclusion = Cookies.get("pl-fitch");
    ...
    clearTree() {
      this.proof = [];
      Cookies.remove("pl-proof");
      this.leaves = [];
      Cookies.remove("pl-leaves");
      this.history = [];
      Cookies.remove("pl-history");
      this.hypotesis = [];
      Cookies.remove("pl-hypo");
      this.fitchProving = false;
      this.conclusion = null;
      Cookies.remove("pl-fitch");
    },
    ...
    addToHistory() {
      ...
      Cookies.set("pl-history", this.history);
    },
    retrieveFromHistory() {
      ...
      Cookies.set("pl-proof", this.proof);
      Cookies.set("pl-leaves", this.leaves);
      Cookies.set("pl-hypo", this.hypotesis);
      Cookies.set("pl-history", this.history);
    },
